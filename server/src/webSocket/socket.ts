...
import { SendChatType, receiverPCType, senderPCType, userType, socketToRoomType } from "../util/type";
...
const getOtherUsersInRoom = (socketID, roomID) => {
  ...
  allUsers = users[roomID].filter((user) => user.id !== socketID).map((otherUser) => ({ id: otherUser.id }));
