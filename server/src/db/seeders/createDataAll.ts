...
console.log("seeders-all-table");

console.log(`
  --------------------------------------
  +++Laggard Project seeders Start+++
  --------------------------------------
`);

let seedAllTable = async () => {
  let seedFiles: string[] = [];
  //? 파일 시스템으로 디렉토리 자체를 읽어온다.
  fs.readdir(path.join(__dirname, "/", "seed"), async (err, files) => {
    ...
    if (files) {
      ...
      for (let el of seedFiles) {
        console.log("seeders File Name : ", el);

        await asyncExec(`./node_modules/.bin/ts-node "${__dirname}/seed/${el}"`).catch((e) => console.log(e));
